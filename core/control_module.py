    ...
    def process_request(self, request):
        ...
        if not is_allowed(role, action):
            ...
            self.logs.create_log(
                ...
                result_preview=None,
                is_blocked=True, blocked_reason="RBAC_DENIED"
            )
            return "DENIED"
        ...
        if detection_status == "ANOMALY":
            print("=" * 55)
            print(f"[⚠ ANOMALY] Agent '{agent_id}' is repeating '{action}' too many times.")
            print(f"[⚠ ANOMALY] Action still executed but flagged in logs.")
            print("=" * 55)
